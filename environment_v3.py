...
for p in range(int(len(all_file_names))):
    ...
    if all_packet_arrival_time[p][-1] >= video_duration + initial_buffers:
        ...
        # Upload the optimal frame size based on the network trace data
        # Since the network is unknown in advance, the transmission time for each frame may exceed 1/fps time
        ...
        for i in range(int(total_frame_number)):
            ...
            # dynamic discount factor algorithm 1
            #if len(network_TP_history) >= 10:
            #    if next_frame_size >= network_TP_history[-1] * frame_time:
            #        discount_factor = 0.8
            #    else:
            #        discount_factor = 1.2

            # dynamic discount factor algorithm 2
            if len(network_TP_history) >= 10:
                if next_frame_size >= network_TP_history[-1] * frame_time:
    ...
    with open('./result/dynamic.txt', 'a+') as f:
