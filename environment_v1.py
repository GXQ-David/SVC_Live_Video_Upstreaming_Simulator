...
for p in range(int(len(all_file_names))): 
        ...
        if ideal_condition == False:
            ...
            for i in range(int(total_frame_number)):
                ...
                for j in range(int(number_of_layers)):
                    ...
                    while flag == 0:
                        if tmp_size - all_instant_speed[p][k] * (all_packet_arrival_time[p][k+1] - tmp_time) > 0:
                            tmp_size = tmp_size - all_instant_speed[p][k] * (all_packet_arrival_time[p][k+1] - tmp_time)
                            ...
                            transmission_time = transmission_time + tmp_size / all_instant_speed[p][k]
                            tmp_time = tmp_time + tmp_size / all_instant_speed[p][k]
                ...
                while flag == 0:
                    if all_packet_arrival_time[p][k+1] <= next_time:
                        estimated_packet_size = estimated_packet_size + all_instant_speed[p][k] * (all_packet_arrival_time[p][k+1] - tmp_time)
                        tmp_time = all_packet_arrival_time[p][k+1]
                    else:
                        estimated_packet_size = estimated_packet_size + all_instant_speed[p][k] * (next_time - tmp_time)
                        break
                    k = k + 1
            ...
            print("")
            print("Instant speed:", statistics.mean(all_instant_speed[p]), "kbps")
            ...
            for i in range(int(total_frame_number)):
                ...
                tmp_time = real_time
                while flag == 0:
                    if all_packet_arrival_time[p][k+1] < (i + 1) * frame_time:
                        estimated_packet_size = estimated_packet_size + all_instant_speed[p][k] * (all_packet_arrival_time[p][k+1] - tmp_time)
                        tmp_time = all_packet_arrival_time[p][k+1]
                    else:
                        estimated_packet_size = estimated_packet_size + all_instant_speed[p][k] * ((i + 1) * frame_time - tmp_time)
                        break
                    k = k + 1

                # calculate how many layers can be sent (knowing the network condition in advance)
                # if a layer cannot be sent within a frame time, then discard it and the following layers
                for j in range(int(number_of_layers)):
                    if uploaded_frame_size + float(data[number_of_layers * i+j]) / 1000 > estimated_packet_size:
                        break
                    else:
                        uploaded_frame_size = uploaded_frame_size + float(data[number_of_layers * i+j]) / 1000
                ...
                tmp_time = real_time
                while flag == 0:
                    if uploaded_frame_size - all_instant_speed[p][k] * (all_packet_arrival_time[p][k+1] - tmp_time) > 0:
                        uploaded_frame_size = uploaded_frame_size - all_instant_speed[p][k] * (all_packet_arrival_time[p][k+1] - tmp_time)
                        transmission_time = transmission_time + all_packet_arrival_time[p][k+1] - tmp_time
                        tmp_time = all_packet_arrival_time[p][k+1]
                    else:
                        transmission_time = transmission_time + uploaded_frame_size / all_instant_speed[p][k]
                        break
                    k = k + 1

                # if network is too bad and nothing is sent
                if transmission_time == 0.0:
                    network_TP_history.append(0.0)
                    frame_cdn_arrival_time.append(real_time)
                else:
                    # under normal network conditon, record the network TP and the frame arrival time
                    real_time = real_time + transmission_time
                    network_TP_history.append(estimated_frame_size[-1] / transmission_time)
                    frame_cdn_arrival_time.append(real_time)
                ...
                if j == number_of_layers:
                    ...
                    print("Starting from the", j + 1, "layer, the following layer(s) are discarded, size =", discarded_layer_size[-1], "kb")
                print("Send time:", frame_send_time[-1], "Arrive time:", real_time)
            
                # record delay and frame loss
                if uploaded_frame_size != 0.0:
                    delay_time.append(real_time - i * frame_time)
                else:
                    frame_loss = frame_loss + 1
            ...
            print("\n")
            print("-------------------------Summary-------------------------\n")
            print("Condition: ideal")
            print("Average delay time:", statistics.mean(delay_time) * 1000, "ms")
            print("Frame loss rate =", frame_loss,"/",int(total_frame_number),"=",float(frame_loss / total_frame_number))
            print("")
            print("Estimated frame size:", statistics.mean(estimated_frame_size), "kb")
            print("Total frame size:", statistics.mean(all_frame_size), "kb")
            print("Discarded layer size:", statistics.mean(discarded_layer_size), "kb")
            print("")
            print("Uploaded percentage: %.2f" % (statistics.mean(estimated_frame_size) / statistics.mean(all_frame_size) * 100))
    ...
    with open('../result/layer_SVC_1.txt', 'w+') as f:
    ...
    with open('../result/network_SVC_1.txt', 'w+') as f:
        for num in range(int(len(delay_time))):
            f.write('%f' % delay_time[num])
            f.write('\n')
    f.close()

    with open('./result/ideal.txt', 'a+') as f:
        tmp1 = statistics.mean(delay_time) * 1000
        tmp2 = statistics.mean(estimated_frame_size)
        tmp3 = sum(estimated_frame_size) / sum(bandwidth_usable) * 100
        tmp4 = frame_loss
